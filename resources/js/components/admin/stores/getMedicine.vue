    ...
    <div style="width: 1000px;margin:auto " overflow-auto>
        ...
        <table>
            ...
            <tr v-for="(medicine,index) in medicines">
                ...
                <td>
                    <img style="width: 50px;height: 50px;border-radius: 50%" :src="medicine.image" alt="">
                </td>
                <td>
                    {{ medicine.description }}
                </td>
                <td>
                    {{ medicine.cost }}.000VND
                </td>
                <td>
                    <el-row>
                        <a :href="'/updateProduct/'+medicine.id">
...
export default {
    ...
    methods: {
        getData(pageNum = 1, keySort = this.keySort) {
            ...
            let scop = this;
            try {
                axios.get('/getMedicine?page=' + pageNum + '&&key=' + keySort)
                    .then((response) => {
                        console.log(response.data);
                        scop.medicines = response.data.data;
