    ...
    <div style="width: 1000px;margin:auto " overflow-auto>
        ...
        <table>
            ...
            <tr v-for="(food,index) in foods">
                ...
                <td>
                    <img style="width: 50px;height: 50px;border-radius: 50%" :src="food.image" alt="">
                </td>
                <td>
                    {{ food.description }}
                </td>
...
export default {
    ...
    methods: {
        getData(pageNum = 1, keySort = this.keySort) {
            ...
            let scop = this;
            try {
                axios.get('/getFood?page=' + pageNum + '&&key=' + keySort)
                    .then((response) => {
                        console.log(response.data);
                        scop.foods = response.data.data;
