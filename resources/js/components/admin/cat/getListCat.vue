    ...
    <div style="width: 1000px;margin:auto " overflow-auto>
        ...
        <table>
            ...
            <tr v-for="(cat,index) in cats">
                ...
                <td>
                    <img style="width: 50px;height: 50px;border-radius: 50%" :src="cat.image" alt="">
                </td>
                <td>
                    {{ cat.description }}
                </td>
...
export default {
    ...
    methods: {
        getData(pageNum = 1, keySort = this.keySort) {
            ...
            let scop = this;
            try {
                axios.get('/getCat?page=' + pageNum + '&&key=' + keySort)
                    .then((response) => {
                        console.log(response.data);
                        scop.cats = response.data.data;
